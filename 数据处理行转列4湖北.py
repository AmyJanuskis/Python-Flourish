...
# 读取Excel数据，选取省份这张表
res = pd.read_excel('data_source4HuBei.xlsx', sheet_name='Sheet1', na_values='0')

res["Date2String"] = res['日期'].apply(lambda x: dt.datetime.strftime(x, '%Y-%m-%d'))
...
days = np.sort( res["Date2String"].unique() )
# 数据集中的省市范围一共有多少个？
provinces = res["省份"].unique()
...
for i in days:
    df[i] = df['province'].apply(lambda x:   safe_list_get( (res[(res.Date2String==i)&(res.省份==x)]["累计确诊"]).tolist(), 0, 0 )       )
print(df.head())
print(df.shape)


df.to_excel('data_result4HuBei.xlsx', sheet_name='data_result_By_Province')
